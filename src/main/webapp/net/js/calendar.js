...
"month":5,
"year":6  };
var flag; //增添全局变量flag，再SelectedDate中初始化，值为undefined 和 false 时表示不可选择将来的时间，为true时表示可以选择将来的时间。
var topY=0,leftX=0;  
...
function SelectDate(obj,strFormat,x,y,flg)
{
if(flg == undefined)
	flag = false;
flag = flg;
leftX = obj.offsetLeft;
topY  = obj.offsetTop;
...
* @param   selected false 表示未来的时间不可选择 true 表示可以选择
* @version 2006-04-01
...
if(calendar.DateMode != pickMode["day"]){
	var selectedDate = new Date(calendar.year,calendar.month,calendar.day,calendar.hour,calendar.minute,calendar.second);
	if(!flag && selectedDate > now)
		today = now;
	else 
		today = selectedDate;
...
//未来的时间显示不同的颜色，光标显示默认形状, 且格子不触发点击事件  2015-06-11 myfu
if(!flag && new Date() < new Date(calendar.date.getFullYear(),
		calendar.date.getMonth(),
		dateArray[i])){
tds[i].style.backgroundColor = calendar.colors["td_word_dark"];
tds[i].bgColorTxt = "td_word_dark";
